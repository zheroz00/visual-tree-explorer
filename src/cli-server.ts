...
function parseArgs(args: string[]): { mode: 'cli' | 'server'; params?: ExploreTreeParamsWithFormat; port?: number } {
  const result: any = { mode: 'cli', params: {} };
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '--server') {
      ...
      result.params.icon_theme = args[i + 1];
      i++;
    ...
    if (pathname === '/explore' && req.method === 'GET') {
      try {
        ...
        const params: ExploreTreeParamsWithFormat = {
          ...
          icon_theme: query.icon_theme as any || 'emoji',
          ...
          format: query.format as any || 'tree'
        };
